...
survivals_cal<-function(patients){
  ...
  # check if the variables are acceptable
  if (length(index)>0){
    # return("One or more variables are not found. Please build file based on the Example CSV.")
    print("One or more variables are not found. Please build file based on the Example CSV.")
  }
  ...
  if (sum(out_range)>0){
    return(paste("One or more values for", num_vars[which(out_range==1)],
                 "are outside of the following range: (", Num_Range[which(out_range==1),1],
                 ",", Num_Range[which(out_range==1),2],")."))
  }else{
    renderText("All numerical values are within the expected range. Now, checking categorical values.")
  }
  
  
  # # fixing some unknown in the datset
  # for (i in 1:nrow(file1)){
  #   for(j in 1:ncol(file1)){
  #     if(!is.na(file1[i,j])){
  #       if(file1[i,j]=="UNKOWN"){file1[i,j]<-"UNKNOWN"}
  #     }
  #     
  #   }
  # }
  ...
  if (sum(CAT_match)>0){
    ...
    renderText("All categorical values are valid. Now, applying logistic regression.")
  }
  ...
  file1.dum <- matrix(NA, ncol=length(all_variables), nrow=10) 
  ...
  # the following is for prediction but I use my matrix
  
  names(all_year_data) <- paste0("Year",0:10)
  survivals <- matrix(NA, ncol=11, nrow=10)
  survival_Probability <- matrix(NA, ncol=11, nrow=10)
  
  predictions<-as.data.frame(matrix(0, ncol = 11, nrow = 10))
  names(predictions)<-c("year0","year1","year2","year3","year4","year5","year6","year7","year8","year9","year10")
  
  # I corrected this, this predicts probability of death
  for(year in 1:11){
    data_predict<-all_year_data[[year]]
    data_predict$Intercept<-1
    for(i in 1:nrow(data_predict)){
      predictions[i,paste("year",year-1,sep="")]<- 1/(
        1+exp(
          -sum(
            as.numeric(log_models[year,names(data_predict)])*as.numeric(data_predict[i,names(data_predict)])
  ...
  # ###############
  # for (i in 11:1){
  #   file2 <- paste0("C:/Users/hza0020/OneDrive - Auburn University/Transplant/BUAL-LAB/shiny/Tessa/final models/all models/model_year",(i-1), ".rds")
  #   fit_model <- readRDS(file2)
  #   temp_result <- predict(fit_model, all_year_data[[i]], type="prob")
  #   survivals[,(i)] <- temp_result$Two
  #   #rm(fit_model)
  # }
  # ##############
  ...
  survivals_isotonic <- t(apply(predictions, 1, function(x) isoreg(x)$yf))
  
  # this calculate proability of survive
  survival_Probability<-1-survivals_isotonic
  
  
  # the survival probability matrix after isotonic regression is applied
  survival_Probability <- cbind(1:10, survival_Probability)
  colnames(survival_Probability) <- c("Patient_No","Month_1 Survival Probability", paste("Year_", 1:10, "Survival Probability"))
